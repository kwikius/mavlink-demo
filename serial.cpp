...
/**
*   Check that a baud rate is valid
*   \param[in] baud The actual baud rate in bits per second e.g 9600
*   \result  True if the baud represents a valid baud rate, else false
*/
   bool check_valid_baud( int baud)
   {
      int const baud_constant = rate_to_constant(baud);
      if (baud_constant == 0){
         return false;
      }
      switch (baud_constant){
...
/**
* Check that the serial_port is in a good state
* \result true if the port is in a good state, else false
*/
...
/**
   ...
   @param[in] baud The actual baud rate in bits per second e.g 9600
*/
void serial_port::init(int baud)
{
    ...
    int const baud_constant = rate_to_constant(baud);
    if ( baud_constant == 0){
      throw std::logic_error("baud constant failed");
    }
    // set baud rate for i/o
    if( cfsetospeed(&new_termios,baud_constant) < 0) {

        this->cleanup();
        throw std::logic_error("open serial port : set baud failed1");
 
    }

    if(cfsetispeed(&new_termios,baud_constant) < 0) {
       // perror("failed to set input baudrate");
        this->cleanup();
        throw std::logic_error("open serial port : set speed failed");
    }
 
    if(tcgetattr(this->m_fd,this->m_old_termios) < 0) {
       // perror(this->m_filename.c_str());
        this->cleanup();
        throw std::logic_error("open serial port : get attr failed");
    }
...
namespace {
   std::clock_t clock_ms(){ return (std::clock() * 1000)/ CLOCKS_PER_SEC;}
}

size_t serial_port::in_avail(uint32_t const & t_ms, size_t n_in )
{

   std::clock_t start_time_ms = clock_ms();
  // printf("start time = %u, %u\n",start_time,);
   
   while ( (clock_ms() - start_time_ms) < t_ms){
...
ssize_t serial_port::write(const char* buf, size_t num) 
{
    return ::write(this->m_fd,reinterpret_cast<const data_type*>(buf),num);
}

bool serial_port::set_pin( unsigned int pin, bool val)
{
...
bool serial_port::set_dtr(bool val)
{
   return set_pin(TIOCM_DTR,val);
}

bool serial_port::set_rts(bool val)
{
...
int serial_port::set_baud( uint32_t baudrate)
{
